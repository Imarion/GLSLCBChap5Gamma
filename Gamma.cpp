...
MyWindow::MyWindow()
    : mProgram(0), currentTimeMs(0), currentTimeS(0), angle(1.74533f), tPrev(0.0f), rotSpeed(M_PI / 8.0f), gamma(2.2f)
{
...
void MyWindow::render()
{
    ...
    mProgram->bind();
    {
        mProgram->setUniformValue("Gamma", gamma);
...
void MyWindow::keyPressEvent(QKeyEvent *keyEvent)
{
    switch(keyEvent->key())
    {
        ...
        case Qt::Key_Down:
            break;
        case Qt::Key_Left:
            if (gamma > 0.1f) gamma -= 0.05f;
            break;
        case Qt::Key_Right:
            if (gamma < 5.0f) gamma += 0.05f;
            break;
        case Qt::Key_Delete:
            break;
